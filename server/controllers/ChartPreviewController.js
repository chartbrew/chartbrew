...
async function loadPreview(chartId, userId) {
  ...
  const chart = await db.Chart.findByPk(chartId, {
    include: [
      { model: db.Project, required: true },
      { model: db.ChartDatasetConfig, include: [db.Dataset] },
    ],
  });
  ...
  if (
    !access.allProjects &&
    (chart.draft ||
      chart.ChartDatasetConfigs.some(
        ({ Dataset: dataset }) =>
          !dataset ||
...
async function placePreview(chartId, targetProjectId, userId) {
  ...
  const { chart, access } = await loadPreview(chartId, userId);
  const target = await db.Project.findOne({
    where: { id: targetProjectId, team_id: access.teamId, ghost: false },
  });
  if (!target) throw createHttpError("Dashboard not found", 404);
  if (!canEditProject(access, target.id))
    throw createHttpError("You cannot add charts to this dashboard", 403);
