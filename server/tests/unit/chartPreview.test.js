...
describe("Signed-in chart previews", () => {
  ...
  it("loads a temporary preview and resolves its current dashboard after placement", async () => {
    ...
    expect(await getPreview(1, 5)).toMatchObject({
      teamId: 7,
      parsed: { visibility: "temporary", dashboard: null },
    });
    chart.Project.ghost = false;
    expect(await getPreview(1, 5)).toMatchObject({
      parsed: { visibility: "dashboard", dashboard: { id: 10 } },
    });
  });
  ...
  it("places a preview and returns the saved dashboard on subsequent loads", async () => {
    ...
    vi.spyOn(db.sequelize, "transaction").mockImplementation(async (callback) =>
    ...
    expect(await getPreview(1, 5)).toMatchObject({
      parsed: { dashboard: { id: 20, name: "Visits" } },
    });
  });
});
