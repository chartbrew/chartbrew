...
function formatRecommendations(recommendations) {
  ...
  recommendations.forEach((recommendation) => {
    ...
    const reasons = (recommendation.reasons || [])
      ...
      .join(" ");
    const learningReason = recommendation.learningReason
      ? `${escapeMarkdown(recommendation.learningReason, 180)} `
      : "";
    lines.push(`- **${escapeMarkdown(recommendation.name, 120)}**${source ? ` — ${source}.` : "."} ${learningReason}${reasons} This metric is not watched yet.`);
  });
