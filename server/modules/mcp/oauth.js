...
const fail = (code = "invalid_request", status = 400) => {
  throw Object.assign(new Error(code), { oauthCode: code, status });
};
const validString = (value, max = 2048) =>
...
function urls() {
  ...
  const resource = new URL(
    process.env.CB_MCP_PUBLIC_URL || new URL("/mcp", apiHost || "http://localhost:3210").href
  );
  if (
    ...
    throw new Error("MCP requires a public HTTPS URL ending in /mcp");
  return {
    resource: resource.href,
    issuer: resource.origin,
    client: settings.client || "http://localhost:4018",
  };
}
...
function scopes(value = "data:read") {
  ...
  if (!result.includes("data:read") || result.some((scope) => !DATA_API_SCOPES.has(scope)))
...
async function register(body) {
  ...
  if (
    !validString(body.client_name || "External app", 80) ||
    [...(body.client_name || "")].some(
      (char) => char.charCodeAt(0) < 32 || char.charCodeAt(0) === 127
    )
  )
    return fail("invalid_client_metadata");
  if (
  ...
  const client = await db.McpOAuthClient.create({
    ...
    name: body.client_name || "External app",
...
async function authenticateClient(req) {
  ...
  if (req.headers.authorization) {
    if (id || supplied || !/^Basic [A-Za-z0-9+/]+=*$/.test(req.headers.authorization))
      return fail("invalid_client", 401);
  ...
  if (
    ...
    (method !== "none" && (!validString(supplied) || !equal(hash(supplied), client.secretHash)))
  )
    return fail("invalid_client", 401);
  return client;
}
...
async function authorize(query) {
  ...
  if (
    !client ||
    !validString(query.redirect_uri) ||
  ...
  if (
    ...
    !validString(query.code_challenge, 43) ||
    !/^[A-Za-z0-9_-]{43}$/.test(query.code_challenge) ||
    (query.state !== undefined && !validString(query.state))
  )
    return fail();
  const resource = requestedResource(query.resource);
  const grant = await db.McpOAuthGrant.create({
    client_id: client.id,
    redirectUri: query.redirect_uri,
    challenge: query.code_challenge,
    state: query.state || null,
    resource,
    scopes: scopes(query.scope === undefined ? [...DATA_API_SCOPES].join(" ") : query.scope),
    expiresAt: after(600),
  });
...
async function pending(id, userId, transaction) {
  ...
  const grant = await db.McpOAuthGrant.findByPk(id, {
    transaction,
  ...
  if (
    ...
    (grant.user_id && grant.user_id !== userId)
  )
...
async function consentInfo(id, userId) {
  return db.sequelize.transaction(async (transaction) => {
    ...
    const roles = await db.TeamRole.findAll({
      where: { user_id: userId, role: { [Op.in]: [...TEAM_WIDE_ROLES, ...PROJECT_ROLES] } },
      include: [{ model: db.Team, attributes: ["id", "name"], required: true }],
      transaction,
    });
    return {
      app: { name: client.name, returnHost: new URL(grant.redirectUri).host },
      scopes: grant.scopes,
      teams: roles.map((role) => ({
        id: role.team_id,
        name: role.Team.name,
        allProjects: TEAM_WIDE_ROLES.has(role.role),
        scopes: allowedScopes(role.role),
      })),
    };
  });
}
...
async function consent(id, userId, body) {
  return db.sequelize.transaction(async (transaction) => {
    ...
    const role = await db.TeamRole.findOne({
      where: { team_id: body.teamId, user_id: userId },
      transaction,
    });
    const selected = scopes(body.scopes.join(" "));
    if (
      !role ||
      ![...TEAM_WIDE_ROLES, ...PROJECT_ROLES].includes(role.role) ||
      selected.some(
        (scope) => !grant.scopes.includes(scope) || !allowedScopes(role.role).includes(scope)
      )
    )
    ...
    await grant.update(
      {
        team_id: body.teamId,
        scopes: selected,
        all_projects: TEAM_WIDE_ROLES.has(role.role),
        project_ids: normalizeProjectIds(role.projects),
        codeHash: hash(code),
        codeExpiresAt: after(60),
        expiresAt: after(90 * 86400),
      },
      { transaction }
    );
...
async function accessForGrant(grant, transaction) {
  ...
  return {
    ...access,
    scopes: access.scopes.filter((scope) => allowedScopes(role.role).includes(scope)),
    teamName: team.name,
...
function signingKey() {
  return createHmac("sha256", settings.encryptionKey).update("chartbrew-mcp-oauth").digest();
}
function decodeAccess(token) {
  if (!validString(token, 4096) || !token.startsWith("cbm_")) return fail("invalid_token", 401);
  const claims = jwt.verify(token.slice(4), signingKey(), {
    algorithms: ["HS256"],
    issuer: urls().issuer,
    audience: urls().resource,
  });
  if (claims.tokenType !== "mcp_oauth" || !validString(claims.sub, 36))
  ...
  const result = await db.sequelize.transaction(async (transaction) => {
    const refresh =
      body.grant_type === "refresh_token"
        ? await db.McpOAuthRefresh.findByPk(hash(credential), { transaction })
        : null;
    const grant = await db.McpOAuthGrant.findOne({
      where: refresh ? { id: refresh.grant_id } : { codeHash: hash(credential) },
      transaction,
    ...
    if (
      ...
      (body.grant_type === "refresh_token" && !refresh)
    )
      return fail("invalid_grant");
    ...
    if (body.grant_type === "authorization_code") {
      if (
        ...
        !validString(body.code_verifier, 128) ||
        !/^[A-Za-z0-9._~-]{43,128}$/.test(body.code_verifier) ||
        !equal(createHash("sha256").update(body.code_verifier).digest("base64url"), grant.challenge)
      )
        return fail("invalid_grant");
      ...
      if (body.scope !== undefined) {
        ...
        if (
          requested.length !== grant.scopes.length ||
...
async function revoke(req) {
  ...
  if (req.body.token.startsWith("cbm_")) {
    try {
      id = decodeAccess(req.body.token).sub;
    } catch {
      return;
    }
  } else {
...
async function cleanupExpiredGrants({ dryRun = false } = {}) {
  ...
  const grants = await db.McpOAuthGrant.findAll({
    attributes: ["id"],
    where,
    limit: 1000,
    order: [["expiresAt", "ASC"]],
  });
  const deleted = grants.length
    ? await db.McpOAuthGrant.destroy({
        where: { id: { [Op.in]: grants.map((grant) => grant.id) } },
      })
    : 0;
