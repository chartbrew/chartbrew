...
function requireSourceAccess(access) {
  ...
  if (!TEAM_WIDE_ROLES.has(access.role) || !access.allProjects)
    throw new DataApiError("API_KEY_SCOPE_REQUIRED");
}

async function searchWorkspace(
  access,
  { query = "", types = ["dashboard", "chart", "dataset"], limit = 20 },
  signal
) {
  const results = [];
  ...
  if (types.includes("dashboard")) {
    const projects = await db.Project.findAll({
      attributes: ["id", "name"],
      where: { id: { [Op.in]: access.projectIds }, team_id: access.teamId, ghost: false, name },
      order: [["id", "ASC"]],
      limit,
    });
    results.push(
      ...projects.map((item) => ({
        ...
        id: item.id,
        name: item.name,
        url: clientLink(`/dashboard/${item.id}`),
      }))
    );
  }
  if (types.includes("chart")) {
      ...
      include: [
        {
          model: db.Project,
          attributes: ["name"],
          where: { team_id: access.teamId, ghost: false },
          required: true,
        },
      ],
      order: [["id", "ASC"]],
    ...
    results.push(
      ...charts.map((item) => ({
        ...
        id: item.id,
        name: item.name,
        dashboard: { id: item.project_id, name: item.Project.name },
        url: clientLink(`/dashboard/${item.project_id}/chart/${item.id}/edit`),
      }))
    );
  }
  if (types.includes("dataset")) {
    ...
    while (count < limit) {
      ...
      const page = await db.Dataset.findAll({
        attributes: ["id", "name", "team_id", "project_ids"],
        where: { id: { [Op.gt]: cursor }, team_id: access.teamId, draft: false, name },
        order: [["id", "ASC"]],
        limit: 100,
      });
      for (const item of page.filter((dataset) => canReadDataset(access, dataset))) {
        results.push({
          ...
          id: item.id,
          name: item.name,
          url: clientLink(`/datasets/${item.id}`),
        });
        count += 1;
...
async function exploreData(access, args) {
  ...
  if (args.operation === "save" && !access.scopes.includes("datasets:write"))
    throw new DataApiError("API_KEY_SCOPE_REQUIRED");
  if (args.operation === "inspect" && !args.connectionId) {
    const connections = await db.Connection.findAll({
      attributes: ["id", "name", "type", "subType"],
      where: { team_id: access.teamId, name: { [Op.like]: `%${args.search || ""}%` } },
      order: [["id", "ASC"]],
      limit: (args.limit || 20) + 1,
    });
    return {
      truncated: connections.length > (args.limit || 20),
      connections: connections.slice(0, args.limit || 20).map((connection) => {
        const source = findSourceForConnection(connection);
        return {
          id: connection.id,
          name: connection.name,
          source: source?.name || connection.type,
          canExplore: Boolean(
            source && isSourceServerEnabled(source) && source.backend?.exploreReadOnly
          ),
        };
      }),
    };
  ...
  if (
    args.operation === "save" &&
    (!args.name?.trim() || !args.projectId || !access.projectIds.includes(args.projectId))
  ) {
    throw new DataApiError("INVALID_REQUEST");
  }
  const connection = await db.Connection.findOne({
    where: { id: args.connectionId, team_id: access.teamId },
  });
  ...
  if (!source || !isSourceServerEnabled(source) || !source.backend?.exploreReadOnly) {
    return {
      status: "unsupported",
      message:
        "Explore this source in Chartbrew and save a dataset first. You can then read it with run_dataset.",
    };
  }
  const preview = await withExecutionDeadline(() =>
    source.backend.exploreReadOnly({ ...args, connection, limit: args.limit || 20 })
  );
  ...
  if (
    ...
    return {
      ...result,
      status: "not_saved",
      message: "The request did not return usable rows. Check the preview before saving a dataset.",
    };
  }
  const project = await db.Project.findOne({
    where: { id: args.projectId, team_id: access.teamId, ghost: false },
    attributes: ["id"],
  });
  if (!project) throw new DataApiError("RESOURCE_NOT_FOUND");
  const dataset = await db.sequelize.transaction((transaction) =>
    datasetController.createWithDataRequests(
      {
        team_id: access.teamId,
        project_ids: [project.id],
        name: args.name.trim(),
        draft: false,
        dataRequests: [{ ...dataRequest, connection_id: connection.id }],
        main_dr_index: 0,
      },
      { transaction }
    )
...
async function createPreview(req, args) {
  ...
  if (
    args.type !== "table" &&
    (!args.yAxis || (!["kpi", "avg", "gauge"].includes(args.type) && !args.xAxis))
  )
    throw new DataApiError("INVALID_REQUEST");
  if (
    ...
    args.ranges?.some((range) => range.min >= range.max)
  )
    throw new DataApiError("INVALID_REQUEST");
  const data = await dataController.datasetData({
    id: req.id,
    method: "POST",
    apiKeyAccess: access,
    body: {},
  ...
  if (!rows.length)
    return {
      result: {
        status: "not_created",
        message: "This dataset has no rows to chart. Check its data and filters first.",
      },
    };
  if (args.type !== "table") {
    const values = rows
      .map((row) => getFieldValue(row, args.yAxis))
      .filter((value) => value != null);
    const countsValues =
      ["count", "count_unique"].includes(args.yAxisOperation) && args.type !== "avg";
    if (
      !values.length ||
      (!countsValues &&
        values.some(
      ...
      return {
        result: {
          status: "not_created",
          message: "Select a numeric value field from run_dataset before creating this chart.",
        },
      };
    }
    if (args.xAxis && rows.every((row) => getFieldValue(row, args.xAxis) == null))
      throw new DataApiError("INVALID_REQUEST");
  }
  ...
  const preview = await createTemporaryChart({
    ...chartOptions,
    spec: chartOptions,
    team_id: access.teamId,
  ...
  if (includeImage) {
    try {
      const png = await withExecutionDeadline(({ signal }) =>
        imageController.render(
          {
            id: req.id,
            user: { id: access.userId },
            params: { chart_id: String(preview.chart_id), project_id: String(preview.project_id) },
            body: { version: 1, layout: "chartOnly" },
          },
          { signal }
        )
      );
      if (png.length <= 1024 * 1024)
        image = { type: "image", mimeType: "image/png", data: png.toString("base64") };
...
async function execute(req, name, args) {
  ...
  if (name === "search_workspace")
    return {
      result: await withExecutionDeadline(({ signal }) => searchWorkspace(access, args, signal)),
    };
  ...
  if (name === "get_workspace_activity") {
    ...
    return {
      result: await withExecutionDeadline(() =>
        readWorkspaceActivity(
          { ...access, allProjects: false },
          {
            ...args,
            observationLimit: args.limit,
            evaluationLimit: args.limit,
            includeHealth: false,
            includeAlerts: false,
          }
        )
  ...
  const response =
    name === "run_dataset"
      ? await dataController.datasetData(request)
      : await dataController.chartData(request);
...
async function callMcpTool(req, name, args) {
  ...
  const summary = {
    ...
    oauthGrantId: access.oauthGrantId,
  };
  const trace = await startRun({
    triggerType: "mcp",
    entityType: "mcp_tool",
    apiKeyId: access.oauthGrantId ? null : access.apiKeyId,
    teamId: access.teamId,
    summary,
  });
  try {
    if (!definition || !access.scopes.includes(definition.scope))
      throw new DataApiError("API_KEY_SCOPE_REQUIRED");
    const { result, image } = await execute(req, name, args);
    if (access.teamName) result.team = { id: access.teamId, name: access.teamName };
    const response = {
      content: [{ type: "text", text: `${name}: ${result.status || "complete"}.` }],
    ...
    const safe =
      error instanceof DataApiError
        ? error
        : new DataApiError(
            error?.code === "EXECUTION_TIMEOUT" ? "EXECUTION_TIMEOUT" : "DATA_UNAVAILABLE",
            { cause: error }
          );
    await failRun(trace, safe, {
      stage: "mcp",
      summary: { ...summary, errorCode: recovery?.code || safe.code },
    });
    if (recovery) {
      let url = null;
      if (recovery.action === "dataset" && recovery.datasetId)
        url = clientLink(`/datasets/${recovery.datasetId}`);
      else if (recovery.action === "connection" && recovery.connectionId)
        url = clientLink(`/connections/${recovery.connectionId}`);
      return {
        isError: true,
        content: [
          { type: "text", text: `${recovery.code}: ${recovery.message}${url ? ` ${url}` : ""}` },
        ],
