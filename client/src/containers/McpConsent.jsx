...
export default function McpConsent() {
  ...
  useEffect(() => {
    ...
    mcpOAuthRequest(`consent/${requestId}`)
      .then((data) => {
        if (!active) return;
        setInfo(data);
      ...
      .catch((failure) => {
        ...
        if (failure.status === 401) navigate(`/login?oauthRequest=${requestId}`, { replace: true });
          ...
          setError(
            "This access request has expired or is not available. Start the connection again in your app."
          );
      });
    return () => {
      active = false;
  ...
  const respond = async (allow) => {
    ...
    setError("");
    try {
      const result = await mcpOAuthRequest(`consent/${requestId}`, {
        method: "POST",
  ...
  return (
    <main className="min-h-screen bg-surface-secondary px-4 py-12 sm:py-20">
      <Helmet>
        <title>Connect an app | Chartbrew</title>
        <meta
          name="referrer"
          content="no-referrer"
        />
        ...
        {info ? (
          <>
            ...
            <p className="mt-3 text-sm text-muted break-words">
              Return address: {info.app.returnHost}
            </p>
            <p className="mt-3 text-sm">
              App names are supplied by the app. Only continue if you started this connection and
              trust this app.
            </p>
            {user.email ? (
              <p className="mt-4 text-sm text-muted break-words">Signed in as {user.email}</p>
            ) : null}
            <Select
              className="mt-6 w-full"
              placeholder="Select a team"
              value={teamId}
              isDisabled={pending || !info.teams.length}
              variant="secondary"
              onChange={(value) => {
            ...
            {!info.teams.length ? (
              ...
              >
                You do not have access to a team. Ask a team owner to invite you, then start the
                connection again.
              </p>
            ) : null}
            {team ? (
              <>
                <p className="mt-3 text-sm text-muted">
                  {team.allProjects
                    ...
                    : "Access is limited to the dashboards you can access now in this team."}
                </p>
                <div
                ...
                >
                  {info.scopes.map((scope) => (
                    <Checkbox
                      key={scope}
                      isSelected={permissions.includes(scope)}
                      isReadOnly={scope === "data:read"}
                      isDisabled={pending || !team.scopes.includes(scope)}
                      onChange={(selected) =>
                        setPermissions((current) =>
                          selected ? [...current, scope] : current.filter((item) => item !== scope)
                        )
                      }
                    >
                      <Checkbox.Content>
                        ...
                        <Label>{MCP_PERMISSIONS[scope]}</Label>
                      </Checkbox.Content>
                    </Checkbox>
                  ))}
                </div>
                {info.scopes.some((scope) => !team.scopes.includes(scope)) ? (
                  <p className="mb-4 text-sm">
                    Your team role does not allow all requested permissions. Only the selected
                    permissions will be granted.
                  </p>
                ) : null}
                <p className="text-sm text-muted">
                  Other teams are not included. You can remove this access in Settings → MCP.
                </p>
              </>
            ) : null}
          </>
