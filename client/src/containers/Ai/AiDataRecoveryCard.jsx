...
function AiDataRecoveryCard({ recovery, onContinue, isLoading }) {
  ...
  const hasTarget =
    ["dataset", "connection"].includes(recovery.action) && /^[1-9]\d*$/.test(String(id));
        ...
        <Card.Footer className="flex-wrap gap-2">
          ...
          {opened || !path ? (
            <Button
              ...
              onPress={() =>
                onContinue(
                  `Check the ${dataset ? "dataset" : "connection"}${hasTarget ? ` ${id}` : ""} again and continue my original request. Do not use old data as a successful refresh.`
                )
              }
            >
