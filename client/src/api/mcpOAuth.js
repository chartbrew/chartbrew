...
export async function mcpOAuthRequest(path, options = {}) {
  const response = await fetch(`${API_HOST}/oauth/${path}`, { ...options, headers: {
    Authorization: `Bearer ${getAuthToken()}`, "Content-Type": "application/json",
  } });
  if (!response.ok) throw Object.assign(new Error("The request could not be completed."), { status: response.status });
